package xyz.e3ndr.localrouter.inference.providers;

import co.casterlabs.rakurai.json.element.JsonObject;
import xyz.e3ndr.localrouter.inference.InferenceProviderType;

public class DeepInfraInferenceProvider extends _OAICompatibleInferenceProvider {

    public DeepInfraInferenceProvider(String id, JsonObject config) {
        super(id, config);
    }

    @Override
    protected String baseUrl() {
        return "https://api.deepinfra.com";
    }

    @Override
    public String v1Prefix() {
        return "/v1/openai";
    }

    @Override
    public InferenceProviderType type() {
        return InferenceProviderType.DEEPINFRA;
    }

}
