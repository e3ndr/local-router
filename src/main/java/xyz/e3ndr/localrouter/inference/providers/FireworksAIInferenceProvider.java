package xyz.e3ndr.localrouter.inference.providers;

import co.casterlabs.rakurai.json.element.JsonObject;
import xyz.e3ndr.localrouter.inference.InferenceProviderType;

public class FireworksAIInferenceProvider extends _OAICompatibleInferenceProvider {

    public FireworksAIInferenceProvider(String id, JsonObject config) {
        super(id, config);
    }

    @Override
    protected String baseUrl() {
        return "https://api.fireworks.ai";
    }

    @Override
    public String v1Prefix() {
        return "/inference/v1";
    }

    @Override
    public InferenceProviderType type() {
        return InferenceProviderType.FIREWORKS_AI;
    }

}
